...
private LinkedListGame gamerA, gamerB;
	...
	public void testNumMatchesTrivial() {
		assertEquals("0 match", 0,
				LinkedListGame.numMatches(1234, 4321));
		assertEquals("0 match", 0,
				LinkedListGame.numMatches(1234, 2341));
	}
	
	@Test(timeout = 1000)
	public void testNumMatches() {
		assertEquals("1 matches", 1,
				LinkedListGame.numMatches(5678, 7777));
		assertEquals("1 match", 1,
				LinkedListGame.numMatches(9000, 3210));
		
		assertEquals("2 matches", 2,
				LinkedListGame.numMatches(1234, 1114));
		assertEquals("2 matches", 2,
				LinkedListGame.numMatches(5005, 6006));

		assertEquals("3 matches", 3,
				LinkedListGame.numMatches(1000, 9000));

		assertEquals("4 matches", 4,
				LinkedListGame.numMatches(1234, 1234));
	}
...
//	@Test(timeout = 1000)
	...
	public void testUpdateGuess() {
		...
		System.out.println(gamerA.priorGuesses().toString());
	}
	
	@Test
	public void testUpdateGuess2() {
		gamerA.reset();
		// ground truth number is 3242
		assertEquals("gamerA first guess", 1000, gamerA.getGuess());
		assertTrue("gamerA first update", gamerA.updateGuess(0));
...
//	@Test(timeout = 1000)
	...
	public void nathanGuess() {
		...
		gamerA.updateGuess(2);
		gamerA.getGuess();	// should be 1010
		gamerA.updateGuess(1);
		System.out.println(gamerA.getGuess());
		assertFalse("state of error", gamerA.updateGuess(1));
